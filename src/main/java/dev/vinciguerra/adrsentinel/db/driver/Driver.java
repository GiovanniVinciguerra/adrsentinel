...
public class Driver {
	...
	@PrePersist
	@PreUpdate
	private void normalize() throws BadRequestException {
		/* Pulizia fullName */
		...
		try {
			...
			PhoneNumber proto = phoneUtil.parse(phone, "IT");
			phone = phoneUtil.format(proto, PhoneNumberUtil.PhoneNumberFormat.E164);
	...
	public String toString() {
		...
		builder.append("Driver [id=").append(id).append(", fullName=").append(fullName).append(", taxCode=")
			.append(taxCode).append(", phone=").append(phone).append(", license=").append(license).append(", cqc=")
			.append(cqc).append(", driverApprovals=").append(driverApprovals).append(", active=").append(active)
			.append("]");
