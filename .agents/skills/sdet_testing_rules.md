...
- **Tag Obbligatori:** Ogni blocco Javadoc a livello di CLASSE deve obbligatoriamente, e senza alcuna eccezione, contenere esattamente questi tre tag:
  ...
  @version 1.0
  @since 1.0
- **Javadoc di Metodo (NESSUNA ECCEZIONE):** **OGNI SINGOLO METODO DI TEST** generato all'interno della classe DEVE possedere un proprio blocco Javadoc professionale. Il Javadoc del metodo deve spiegare chiaramente: l'intento logico e di business del test (es. perché una combinazione ADR fallisce), quali mock intervengono nel setup e l'output o l'eccezione attesa. Ti è vietato omettere questo blocco su qualsiasi metodo.
